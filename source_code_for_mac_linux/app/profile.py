...
class Inventory:
    ...
    def add(self, content: str) -> None:
        ...
        try:
            # Handle standard format " <:emoji:id> Item Name"
            if ' <' in content:
                parts = sub(r' <.+?> ', '#', content).split('#')
                if len(parts) == 2:
                    amount, name = parts
                else:
                    # Fallback if split produces unexpected results
                    amount = "1"
                    name = content.strip()
            else:
                # Handle cases without emoji or unexpected format
                amount = "1"
                name = content.strip()
            ...
            # Fallback for any parsing error to prevent crash
            self.items.append(("1", content))
        return None
