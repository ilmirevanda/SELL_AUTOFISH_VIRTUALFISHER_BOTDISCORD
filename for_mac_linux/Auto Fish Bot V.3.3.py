                ...
                if self.captcha.detected and not self.captcha.regenerating:
                    if self.message.content == 'You may now continue.':
                        ...
                        if msg_text.find('incorrect code') > -1:
                            ...
                            # Check if we are stuck (Detected, not solving, no answers, no image/text)
                            if not self.captcha.solving and not self.captcha.answers and not self.captcha.captcha_image and not self.captcha.captcha_text:
                                if self.captcha.regens < MAX_CAPTCHA_REGENS:
                                    self.menu.notify(f'[!] STUCK: Captcha detected but image missing. Forcing regen ({self.captcha.regens + 1}/{MAX_CAPTCHA_REGENS})...', NotificationPriority.HIGH)
                                    self.captcha.regens += 1
                                    self.captcha.regenerating = True
