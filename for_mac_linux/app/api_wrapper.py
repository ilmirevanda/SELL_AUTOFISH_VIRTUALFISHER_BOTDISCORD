...
NON_CRITICAL_CODES = [400, 429, 500, 502, 503, 504]
...
class Proxy:
    ...
    def validate(self) -> bool:
        ...
        for url in self.validator_urls:
            ...
            try:
                request = get(url, proxies=self.proxies(), auth=self.auth(), timeout=self._max_timeout, headers={'User-Agent': DEFAULT_USER_AGENT})
...
class DiscordWrapper:
            ...
            if status_code in NON_CRITICAL_CODES:
                ...
                if status_code == 400:
                    ...
                    if int(response.get('code', 0)) == INVALID_FORM_BODY:
                        ...
                        # Fix for text captcha 50035 error: 
                        # This happens when button interaction fails (invalid component),
                        # usually because the button is no longer valid or we tried to verify text captcha via button logic.
                        # We should just return False to let the loop retry with slash command if needed.
                        return False
                    else:
                        #Critial, debbug and raise exception - ax1
                        debugger.log(err_message, 'ax1')
                        # LOG BEFORE RAISE
                        try:
                            with open('debug_exit.log', 'a') as f:
                                f.write(f"[{datetime.now()}] [ERROR] AX1 ERROR (Raised Exception): {err_message}\n")
                        except: pass
                        raise Exception(f"AX1 Error: {err_message}")
                ...
                #Rate limits
                    ...
                    try:
                        if not response['global']:
                            ...
                            message = response['message']
                            raise Exception(f'[E] 429 (Global): {message} {data}')
                    except (KeyError, TypeError, ValueError) as e:
                        debugger.log(e, f'{self.name} 429')
                        raise Exception(f'[E] 429: {e}')
                else:
                    #Gateway unavailable
                ...
                try:
                    with open('debug_exit.log', 'a') as f:
                        f.write(f"[{datetime.now()}] [ERROR] Status code error: {status_code}\n")
                except: pass
                raise Exception(f'[E] Status code: {status_code}')
        else:
                ...
                for guild in event['d']['guilds']:
                    for channel in guild['channels']:
                        if str(channel['id']) == self.channel_id:
                            return str(guild['id'])
                raise Exception('[E] Incorrect channel id.')
            else:
                pass
    ...
    def get_me(self) -> None:
        ...
        try:
            content = self.request(endpoint='https://discord.com/api/v10/users/@me', method='get')
            if content and isinstance(content, dict) and 'username' in content:
                self.username = content['username']
            else:
                self.username = "Unknown"
        except Exception as e:
            debugger.log(e, f'{self.name} - get_me')
            self.username = "Unknown"
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if proxy_host:
                if 'socks5://' in proxy_host:
                    proxy_type = 'socks5'
                    proxy_host = proxy_host.replace('socks5://', '')
                elif 'socks4://' in proxy_host:
                    proxy_type = 'socks4'
                    proxy_host = proxy_host.replace('socks4://', '')
                elif 'http://' in proxy_host:
                    proxy_host = proxy_host.replace('http://', '')
                elif 'https://' in proxy_host:
                    proxy_host = proxy_host.replace('https://', '')
            
            self.ws.connect(self.endpoints.gateway, http_proxy_host=proxy_host, 
                            http_proxy_port=proxy_port, http_proxy_auth=proxy_auth, proxy_type=proxy_type)
            ...
            try:
                with open('debug_exit.log', 'a') as f:
                    f.write(f"[{datetime.now()}] [ERROR] CONNECT FAILURE (Raised): {e}\n")
            except: pass
            raise Exception(f'[!] Failed to connect to gateway -> {e}')
    ...
    def reconnect(self) -> bool:
        ...
        if self.is_connected:
            if not self.disconnect():
                ...
                try:
                    with open('debug_exit.log', 'a') as f:
                        f.write(f"[{datetime.now()}] [ERROR] RECONNECT-DISCONNECT FAILURE (Raised)\n")
                except: pass
                raise Exception("[E] Failed to reconnect.")
