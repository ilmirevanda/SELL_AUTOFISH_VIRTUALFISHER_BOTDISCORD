...
class MainMenu(BaseMenu):
    ...
    def get_inventory_panel(self) -> Panel:
        ...
        # 3. Exotic Fish Table (Added to show hidden data)
        # Check if we have any exotic fish
        has_exotic = False
        if self.profile.exotic_fish.list:
            for count, name in self.profile.exotic_fish.list:
                if isinstance(count, int) and count > 0:
                    has_exotic = True
                    break
        
        if has_exotic:
            main_grid.add_row(Text("-" * 30, style="dim white", justify="center"))
            exotic_header = Text("Exotic Fish", style="bold purple", justify="center")
            main_grid.add_row(exotic_header)
            
            exotic_table = Table(box=None, expand=True, show_header=False)
            exotic_table.add_column("Fish", style="purple")
            exotic_table.add_column("Count", style="green", justify="right")
            
            for count, name in self.profile.exotic_fish.list:
                if count > 0:
                    exotic_table.add_row(str(name), str(count))
            
            main_grid.add_row(exotic_table)
